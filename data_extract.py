...
split_index = int(total_files * 0.9) # percentage of data to be set for training can be changed changing the value of this variable
files_train = files[:split_index]
files_val = files[split_index:]

#Sampling a hundereds oof the files for each split 
sample_rate = 0.01
...
vocab_train = process_file_in_parallel(files_train_sampled, folder_path, output_file_train)

#Process the sampled validation files
vocab_val = process_file_in_parallel(files_val_sampled, folder_path, output_file_val)

#combine vocabularies (if needed) to write to vocab.txt
